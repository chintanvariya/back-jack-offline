...
{
    public class BlackjackStoreIconProvider
    {
        ...
        public static void Initialize(ProductCollection Products)
        {
            if (Icons.Count == 0)
            {
                ...
                {
                    Debug.Log($"Loading store icons for products ID : {product.definition.id} .");
                    // Debug.Log($"Loading store icon at path ProductIcons/{product.definition.id}");
                    ResourceRequest operation = Resources.LoadAsync<Texture2D>($"ProductIcons/{product.definition.id}");
